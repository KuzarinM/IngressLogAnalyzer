<script>
    ...
    export default {
        name: 'SidebarList',
        props: {
            locations: Array,        // Отфильтрованный список (для отображения)
            uniqueServices: Array,   // Список сервисов
            globalMinTime: Number,   // Самое раннее время в логах (timestamp)
            globalMaxTime: Number    // Самое позднее время в логах (timestamp)
        },
        ...
        computed: {
            // Вычисляем строки границ для атрибутов min/max инпутов
        ...
        watch: {
            ...
            // ИЗМЕНЕНИЕ: immediate: true гарантирует запуск при старте
            globalMinTime: {
                immediate: true,
                handler(newVal) {
                    if (newVal && newVal > 0) {
        ...
        methods: {
            ...
            formatDateForInput(timestamp) {
                // Если timestamp 0, NaN или null — возвращаем пустую строку
    ...
    <template>
        <aside class="w-80 bg-white border-r border-gray-200 flex flex-col z-10 shadow-lg shrink-0">
            ...
            <div class="p-3 border-b bg-gray-50 space-y-3">
                ...
                <!-- Поиск -->
                <input type="text" v-model="searchQuery" placeholder="Поиск (IP, Страна)..." class="w-full px-3 py-1.5 border rounded text-sm focus:outline-none focus:border-blue-500">
